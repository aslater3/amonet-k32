#!/usr/bin/env bash
# Deploy the K32-specific Amonet wrapper and reviewed v184 stock ARM32 image.
#
...
K32_WRAPPER_IMAGE="${K32_WRAPPER_IMAGE:-$WORKDIR/bin/boot-k32-wrapper.sparse.img}"
K32_WRAPPER_SHA256="${K32_WRAPPER_SHA256:-b9a9dc0549b117a600ae497ce3f569171bb08fd94a35e225c04e784927403184}"
...
python3 "$WORKDIR/amonet-k32/verify-post-payload.py"
...
if (( preflight_only )); then
  printf 'v184 stock ARM32 + K32 Amonet preflight PASS (no device access)\n'
  exit 0
fi
...
printf 'v184 stock ARM32 + K32 Amonet fastboot preflight PASS\n'
...
printf '4. reboot (preserve BROM-initialized BCB)\n'
