...
# This serial port is the BROM/preloader USB-CDC interface (/dev/ttyACM*).
# Keep BROM at 115200.  The separate CH340 debug console (/dev/ttyUSB0)
# is configured independently at 921600 by the UART capture tooling.
BROM_BAUD = 115200
BAUD = BROM_BAUD
TIMEOUT = 5
...
class Device:
    ...
    def read_reg(self, addr):
        # magic
        self.dev.write(p32_be(0xf00dd00d))
        # cmd = 0x4000 (read 32-bit register, no MPU in BROM)
        self.dev.write(p32_be(0x4000))
        # address
        self.dev.write(p32_be(addr))
        # payload replies with one dword
        data = self.dev.read(4)
        if len(data) != 4:
            raise RuntimeError("read_reg fail")
